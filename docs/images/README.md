...
All three stages running successfully — Source ✅ → Build ✅ → Deploy ✅
...
![CodeBuild logs](docs/images/codebuild-logs.png)

All phases complete: INSTALL → BUILD → POST_BUILD → UPLOAD_ARTIFACTS — all `SUCCEEDED`.
...
React application served directly from S3 static website hosting at:
http://react-app-dora-2026.s3-website.eu-west-2.amazonaws.com

---

## S3 bucket — deployed files

![S3 bucket](docs/images/s3-bucket.png)

CodePipeline deploys the React build output directly into S3 — `index.html`, `static/` folder, manifests, and all assets. Every push replaces these files automatically.
